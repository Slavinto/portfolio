...
export async function finishGame(
    ...
    try {
        const user = await isUserLoggedIn();
        const { data: originalGame, error: gameError } = await getGameById(
            gameId
        );
        ...
        if (originalGame.status === "ongoing") {
            // updating games table winner id
        ...
        // updating if player looses
        else if (winnerId !== null && user.id !== winnerId) {
            await supabase.rpc("increment_player_stats", {
                p_player_id: user.id,
                p_result: "loss",
            });
        }
        // updating in case of a draw
        else {
            await supabase.rpc("increment_player_stats", {
                p_player_id: user.id,
                p_result: "draw",
